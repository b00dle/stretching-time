...
class KeyboardInput(avango.script.Script):
    ...
    ## scale values
    sf_scale_value0 = avango.SFFloat()
    ...
    def __init__(self):
        ...
        self.scale_velocity = 0.01 # in normalized percent
        ...
        # scale button connections
        self.sf_button7.connect_from(self.keyboard_sensor.Button16) # u key       
        self.sf_button8.connect_from(self.keyboard_sensor.Button17) # i key       
    ...
    def evaluate(self): # evaluated every frame if any input field has changed  
        ...
        frame_elapsed = time.time() - self.lf_time 
        self.lf_time = time.time() # save absolute time of last frame (required for frame-rate independent mapping)
        scale = frame_elapsed / (1.0/60.0)
                
        ## get rot_value0
        if self.sf_button0.value == True:
            #print("- 0/1")
            self.sf_rot_value0.value = scale * self.rot_velocity * -1.0

        elif self.sf_button1.value == True:
            #print("+ 0/1")
            self.sf_rot_value0.value = scale * self.rot_velocity
        ...
        if self.sf_button2.value == True:
            #print("- 2/3")
            self.sf_rot_value1.value = scale * self.rot_velocity * -1.0

        elif self.sf_button3.value == True:
            #print("+ 2/3")
            self.sf_rot_value1.value = scale * self.rot_velocity
        ...
        if self.sf_button4.value == True:
            #print("- 4/5")
            self.sf_rot_value2.value = scale * self.rot_velocity * -1.0

        elif self.sf_button5.value == True:
            #print("+ 4/5")
            self.sf_rot_value2.value = scale * self.rot_velocity
        
        else:
            self.sf_rot_value2.value = 0.0

        ## get sf_scale_value0
        if self.sf_button7.value == True:
            self.sf_scale_value0.value = scale * self.scale_velocity * -1.0

        elif self.sf_button8.value == True:
            self.sf_scale_value0.value = scale * self.scale_velocity
                    
        else:
            self.sf_scale_value0.value = 0.0